...
/**
  ...
  * @param  Line   行位置，范围：1~4
  * @param  Column 列位置，范围：1~8（中文占2列宽度，每个中文16像素）
  * @param  Index  中文在字库中的索引
  * @retval 无
  */
void OLED_ShowChinese(uint8_t Line, uint8_t Column, uint8_t Index)
{
    ...
    for (i = 0; i < 16; i++)
    {
        OLED_WriteData(OLED_CN16x16[Index][i]);                 //显示上半部分内容
    }
    OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 16);     //设置光标位置在下半部分
    for (i = 0; i < 16; i++)
    {
        OLED_WriteData(OLED_CN16x16[Index][i + 16]);           //显示下半部分内容
    }
}
...
/**
  * @brief  OLED显示一行中文
  * @param  Line    行位置，范围：1~4
  * @param  Column  起始列位置，范围：1~8（中文占2列，每个中文16像素）
  * @param  Index   中文索引数组（如 {0,1} 表示"距离"）
  * @param  len     中文个数
  * @retval 无
  */
void OLED_ShowChineseStr(uint8_t Line, uint8_t Column, const uint8_t Index[], uint8_t len)
{
    ...
    for (i = 0; i < len; i++)
    {
        OLED_SetCursor((Line - 1) * 2, (Column - 1 + i * 2) * 8);   //每个中文占2列（16像素）
        for (j = 0; j < 16; j++)
        {
            OLED_WriteData(OLED_CN16x16[Index[i]][j]);
        }
        OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1 + i * 2) * 8);
        for (j = 0; j < 16; j++)
        {
            OLED_WriteData(OLED_CN16x16[Index[i]][j + 16]);
        }
    }
}
