...
float imu963ra_transition_factor[3] = {4098, 14.3f, 3000};

#define imu963ra_delay_ms(x) R_BSP_SoftwareDelay(x, BSP_DELAY_UNITS_MILLISECONDS)
...
#else
#define IMU963RA_CS(x)              (g_ioport.p_api->pinWrite(&g_ioport_ctrl, IMU963RA_CS_PIN, x))

volatile bool imu963ra_spi_transfer_finish = false;
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     IMU963RA 写寄存器
// 参数说明     reg             寄存器地址
// 参数说明     data            数据
...
{
    uint8 buffer[2] = {reg | IMU963RA_SPI_W, data};
    IMU963RA_CS(0);
    g_sci_spi0.p_api->write(g_sci_spi0.p_ctrl, &buffer, 2, SPI_BIT_WIDTH_8_BITS);
    while(!imu963ra_spi_transfer_finish);
    imu963ra_spi_transfer_finish = false;
    IMU963RA_CS(1);
}
...
{
    uint8 buffer[2] = {reg | IMU963RA_SPI_R, 0xFF};
    IMU963RA_CS(0);
...
{
    uint8 buffer[8] = {reg | IMU963RA_SPI_R, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    IMU963RA_CS(0);
...
static void imu963ra_spi_callback (spi_callback_args_t* p_args)
{
    switch(p_args->event)
    {
        case SPI_EVENT_TRANSFER_COMPLETE:
            imu963ra_spi_transfer_finish = true;
            break;
        default:
            break;
    }
...
uint8 imu963ra_init (void)
{
    ...
    if(!init_flag)
    {
        ...
        g_sci_spi0.p_api->callbackSet(g_sci_spi0.p_ctrl, imu963ra_spi_callback, NULL, NULL);
    }
#endif
    ...
    do
    {
        ...
        imu963ra_delay_ms(20);                                                    // 等待磁力计获取数据
    }while(0);
    return return_state;
}
