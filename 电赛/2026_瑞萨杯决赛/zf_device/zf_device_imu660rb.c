...
float imu660rb_transition_factor[2] = {4098, 14.3f};                  // 转换实际值的比例

#define imu660rb_delay_ms(x) R_BSP_SoftwareDelay(x, BSP_DELAY_UNITS_MILLISECONDS)
...
#else
#define IMU660RB_CS(x)              (g_ioport.p_api->pinWrite(&g_ioport_ctrl, IMU660RB_CS_PIN, x))

volatile bool imu660rb_spi_transfer_finish = false;
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     IMU660RB 写寄存器
// 参数说明     reg             寄存器地址
// 参数说明     data            数据
...
{
    uint8 buffer[2] = {reg | IMU660RB_SPI_W, data};
    IMU660RB_CS(0);
    g_sci_spi0.p_api->write(g_sci_spi0.p_ctrl, &buffer, 2, SPI_BIT_WIDTH_8_BITS);
    while(!imu660rb_spi_transfer_finish);
    imu660rb_spi_transfer_finish = false;
    IMU660RB_CS(1);
}
...
{
    uint8 buffer[2] = {reg | IMU660RB_SPI_R, 0xFF};
    IMU660RB_CS(0);
...
{
    uint8 buffer[8] = {reg | IMU660RB_SPI_R, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    IMU660RB_CS(0);
...
static void imu660rb_spi_callback (spi_callback_args_t* p_args)
{
    switch(p_args->event)
    {
        case SPI_EVENT_TRANSFER_COMPLETE:
            imu660rb_spi_transfer_finish = true;
            break;
        default:
            break;
    }
...
uint8 imu660rb_init (void)
{
    ...
    if(!init_flag)
    {
        ...
        g_sci_spi0.p_api->callbackSet(g_sci_spi0.p_ctrl, imu660rb_spi_callback, NULL, NULL);
    }
#endif
