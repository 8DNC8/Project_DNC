...
#define imu660ra_delay_ms(x) R_BSP_SoftwareDelay(x, BSP_DELAY_UNITS_MILLISECONDS)
...
#else
#define IMU660RA_CS(x)              (g_ioport.p_api->pinWrite(&g_ioport_ctrl, IMU660RA_CS_PIN, x))

volatile bool imu660ra_spi_transfer_finish = false;
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     IMU660RA 写寄存器
// 参数说明     reg             寄存器地址
// 参数说明     data            数据
...
{
    uint8 buffer[2] = {reg | IMU660RA_SPI_W, data};
    IMU660RA_CS(0);
    g_sci_spi0.p_api->write(g_sci_spi0.p_ctrl, &buffer, 2, SPI_BIT_WIDTH_8_BITS);
    while(!imu660ra_spi_transfer_finish);
    imu660ra_spi_transfer_finish = false;
    IMU660RA_CS(1);
}
...
{
    IMU660RA_CS(0);
    g_sci_spi0.p_api->write(g_sci_spi0.p_ctrl, &reg, 1, SPI_BIT_WIDTH_8_BITS);
    while(!imu660ra_spi_transfer_finish);
    imu660ra_spi_transfer_finish = false;
    g_sci_spi0.p_api->write(g_sci_spi0.p_ctrl, data, len, SPI_BIT_WIDTH_8_BITS);
    while(!imu660ra_spi_transfer_finish);
    imu660ra_spi_transfer_finish = false;
    IMU660RA_CS(1);
}
...
{
    uint8 buffer[3] = {reg | IMU660RA_SPI_R, 0xFF, 0xFF};
    IMU660RA_CS(0);
    g_sci_spi0.p_api->writeRead(g_sci_spi0.p_ctrl, &buffer, &buffer, 3, SPI_BIT_WIDTH_8_BITS);
    while(!imu660ra_spi_transfer_finish);
    imu660ra_spi_transfer_finish = false;
    IMU660RA_CS(1);
    return buffer[2];
...
{
    uint8 buffer[8] = {reg | IMU660RA_SPI_R, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    IMU660RA_CS(0);
    g_sci_spi0.p_api->writeRead(g_sci_spi0.p_ctrl, &buffer, &buffer, len + 2, SPI_BIT_WIDTH_8_BITS);
    while(!imu660ra_spi_transfer_finish);
    imu660ra_spi_transfer_finish = false;
    IMU660RA_CS(1);
...
static void imu660ra_spi_callback (spi_callback_args_t* p_args)
{
    switch(p_args->event)
    {
        case SPI_EVENT_TRANSFER_COMPLETE:
            imu660ra_spi_transfer_finish = true;
            break;
        default:
            break;
    }
...
uint8 imu660ra_init (void)
{
    ...
    if(!init_flag)
    {
        ...
        g_sci_spi0.p_api->callbackSet(g_sci_spi0.p_ctrl, imu660ra_spi_callback, NULL, NULL);
    }
...
void sci0_b_spi_callback (spi_callback_args_t* p_args)
{
    switch(p_args->event)
    {
        default:
