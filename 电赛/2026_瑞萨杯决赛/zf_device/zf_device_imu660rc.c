...
#define imu660rc_delay_ms(x) R_BSP_SoftwareDelay(x, BSP_DELAY_UNITS_MILLISECONDS)
...
#else
#define IMU660RC_CS(x)              (g_ioport.p_api->pinWrite(&g_ioport_ctrl, IMU660RC_CS_PIN, x))

volatile bool imu660rc_spi_transfer_finish = false;
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     IMU660RC 写寄存器
// 参数说明     reg             寄存器地址
// 参数说明     data            数据
...
{
    uint8 buffer[2] = {reg | IMU660RC_SPI_W, data};
    IMU660RC_CS(0);
    g_sci_spi0.p_api->write(g_sci_spi0.p_ctrl, &buffer, 2, SPI_BIT_WIDTH_8_BITS);
    while(!imu660rc_spi_transfer_finish);
    imu660rc_spi_transfer_finish = false;
    IMU660RC_CS(1);
}
...
{
    uint8 buffer[2] = {reg | IMU660RC_SPI_R, 0xFF};
    IMU660RC_CS(0);
    g_sci_spi0.p_api->writeRead(g_sci_spi0.p_ctrl, &buffer, &buffer, 2, SPI_BIT_WIDTH_8_BITS);
    while(!imu660rc_spi_transfer_finish);
    imu660rc_spi_transfer_finish = false;
    IMU660RC_CS(1);
    return buffer[1];
...
{
    uint8 buffer[8] = {reg | IMU660RC_SPI_R, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    IMU660RC_CS(0);
    g_sci_spi0.p_api->writeRead(g_sci_spi0.p_ctrl, &buffer, &buffer, len + 1, SPI_BIT_WIDTH_8_BITS);
    while(!imu660rc_spi_transfer_finish);
    imu660rc_spi_transfer_finish = false;
    IMU660RC_CS(1);
...
//
//-------------------------------------------------------------------------------------------------------------------
void imu660rc_callback (external_irq_callback_args_t * arg)
{
    if(IMU660RC_INT2_PIN == arg->channel)
    {
...
static void imu660rc_spi_callback (spi_callback_args_t* p_args)
{
    switch(p_args->event)
    {
        case SPI_EVENT_TRANSFER_COMPLETE:
            imu660rc_spi_transfer_finish = true;
            break;
        default:
            break;
    }
...
uint8 imu660rc_init(imu660rc_quarternion_rate_config quarternion_rate)
{
    ...
    imu660rc_quarternion_rate = quarternion_rate;
        ...
        {
            init_flag = 1;
            g_sci_spi0.p_api->open(g_sci_spi0.p_ctrl, g_sci_spi0.p_cfg);
            g_sci_spi0.p_api->callbackSet(g_sci_spi0.p_ctrl, imu660rc_spi_callback, NULL, NULL);
        }
    #endif

        imu660rc_delay_ms(10);
    ...
    do
    {
        ...
        if(IMU660RC_QUARTERNION_DISABLE != quarternion_rate)
        {
			...
			// 开启中断检测引脚
            g_external_irq25.p_api->open(g_external_irq25.p_ctrl, g_external_irq25.p_cfg);
            g_external_irq25.p_api->callbackSet(g_external_irq25.p_ctrl, imu660rc_callback, NULL, NULL);
            g_external_irq25.p_api->enable(g_external_irq25.p_ctrl);
        }
    }while(0);
