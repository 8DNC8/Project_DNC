...
/* FSP 的 SPI write 是非阻塞的（启动中断后就返回，真正的传输在后台由中断完成）。
 * 但 IPS200 驱动是按“阻塞 SPI”写的，所以这里加一个完成标志，写完数据才返回，
 * 和 zf_device_imu660rb.c 里 SPI0 的处理方式保持一致，避免连续写数据丢失导致花屏/卡死。 */
static volatile bool ips200_spi_transfer_finish = false;

static void ips200_write_8bit_data_spi(uint8 data)
{
    g_sci_spi5.p_api->write(g_sci_spi5.p_ctrl, &data, 1, SPI_BIT_WIDTH_8_BITS);
    while(!ips200_spi_transfer_finish);
    ips200_spi_transfer_finish = false;
}

static void ips200_write_8bit_data_spi_array(uint8 *ptr, uint32 len)
{
    g_sci_spi5.p_api->write(g_sci_spi5.p_ctrl, ptr, len, SPI_BIT_WIDTH_8_BITS);
    while(!ips200_spi_transfer_finish);
    ips200_spi_transfer_finish = false;
}
...
{
    uint8 temp[2] = {(uint8)(data >> 8), (uint8)(data >> 0)};
...
static void ips200_write_16bit_data_spi_array (uint16 *ptr, uint32 len)
{
    ...
    {
        write_16bit_data_temp[i + 0] = (uint8)(*ptr >> 8);
        write_16bit_data_temp[i + 1] = (uint8)(*ptr >> 0);
        ptr ++;
    }
    ...
    do
    {
        ...
        if(func_abs(y_start - y_end) > func_abs(x_start - x_end))
        {
            while(y_start != y_end)
            {
                ips200_draw_point(x_start, y_start, color);
                y_start = (uint16)(y_start + y_dir);
                x_start = (uint16)(((float)y_start - temp_b) / temp_rate);
            }
            ips200_draw_point(x_start, y_start, color);
        }
        else
        {
            ...
            {
                ips200_draw_point(x_start, y_start, color);
                x_start = (uint16)(x_start + x_dir);
                y_start = (uint16)((float)x_start * temp_rate + temp_b);
            }
            ips200_draw_point(x_start, y_start, color);
        }
    }while(0);
...
// 参数说明     x1,y1           左上角坐标
// 参数说明     x2,y2           右下角坐标
// 参数说明     color           颜色 RGB565
// 返回参数     void
// 使用示例     ips200_draw_rect(10, 10, 50, 50, RGB565_RED);
//-------------------------------------------------------------------------------------------------------------------
...
// 函数简介     IPS200 画十字准星
// 参数说明     x,y             中心坐标
// 参数说明     half_size       半长（像素）
// 参数说明     color           颜色 RGB565
// 返回参数     void
// 使用示例     ips200_draw_cross(80, 60, 5, RGB565_GREEN);
//-------------------------------------------------------------------------------------------------------------------
void ips200_draw_cross (uint16 x, uint16 y, uint16 half_size, const uint16 color)
{
    uint16 x_start = (x > half_size) ? (x - half_size) : 0;
    uint16 x_end   = (x + half_size < ips200_x_max) ? (x + half_size) : (ips200_x_max - 1);
    uint16 y_start = (y > half_size) ? (y - half_size) : 0;
    uint16 y_end   = (y + half_size < ips200_y_max) ? (y + half_size) : (ips200_y_max - 1);

    ips200_draw_line(x_start, y, x_end, y, color);
    ips200_draw_line(x, y_start, x, y_end, color);
}
...
void ips200_show_string (uint16 x, uint16 y, const char dat[])
{
    ...
    while('\0' != dat[j])
    {
        switch(ips200_display_font)
        {
            case IPS200_6X8_FONT:   ips200_show_char((uint16)(x + 6u * j), y, dat[j]); break;
            case IPS200_8X16_FONT:  ips200_show_char((uint16)(x + 8u * j), y, dat[j]); break;
...
void ips200_show_float (uint16 x, uint16 y, const double dat, uint8 num, uint8 pointnum)
{
    ...
    memset(data_buffer, ' ', (size_t)(num + pointnum + 2u));
...
void ips200_show_binary_image (uint16 x, uint16 y, const uint8 *image, uint16 width, uint16 height, uint16 dis_width, uint16 dis_height)
{
    ...
    ips200_set_region(x, y, (uint16)(x + dis_width - 1u), (uint16)(y + dis_height - 1u));             // 设置显示区域
...
/* 单个 RGB565 像素的 r/g/b 通道平均（避免直接对整值求平均导致偏色） */
static uint16 ips200_rgb565_channel_avg2(uint32 c0, uint32 c1)
{
    uint32 r = (((c0 >> 11) & 0x1F) + ((c1 >> 11) & 0x1F)) / 2;
    uint32 g = (((c0 >> 5) & 0x3F) + ((c1 >> 5) & 0x3F)) / 2;
    uint32 b = ((c0 & 0x1F) + (c1 & 0x1F)) / 2;
    return (uint16)((r << 11) | (g << 5) | b);
}

static uint16 ips200_rgb565_channel_avg4(uint32 c0, uint32 c1, uint32 c2, uint32 c3)
{
    uint32 r = (((c0 >> 11) & 0x1F) + ((c1 >> 11) & 0x1F) + ((c2 >> 11) & 0x1F) + ((c3 >> 11) & 0x1F)) / 4;
    uint32 g = (((c0 >> 5) & 0x3F) + ((c1 >> 5) & 0x3F) + ((c2 >> 5) & 0x3F) + ((c3 >> 5) & 0x3F)) / 4;
    uint32 b = ((c0 & 0x1F) + (c1 & 0x1F) + (c2 & 0x1F) + (c3 & 0x1F)) / 4;
    return (uint16)((r << 11) | (g << 5) | b);
}

/* 凌瞳(SCC8660)输出的 RGB565 在内存里是"高字节在前"（字节交换，所以显示要用
 * ips200_show_rgb565_image 的 color_mode=1 直接按字节写）。这里在求平均前先把
 * 像素换回标准 RGB565（r 在高 5 位），算完再换回"高字节在前"的内存字节序写屏，
 * 否则插值出来的像素颜色会花掉。两个方向是同一个操作（自逆）。 */
static uint16 ips200_rgb565_mem_to_std(uint32 v)
{
    return (uint16)(((v & 0xFFu) << 8) | (v >> 8));
}

static uint16 ips200_rgb565_std_to_mem(uint32 v)
{
    return (uint16)(((v & 0xFFu) << 8) | (v >> 8));
}

//-------------------------------------------------------------------------------------------------------------------
// 函数简介     IPS200 平滑 2 倍放大显示 RGB565 图像
// 参数说明     x,y             显示起点坐标
// 参数说明     *image          图像数组指针（原图尺寸 width x height）
// 参数说明     width           图像实际宽度
// 参数说明     height          图像实际高度
// 返回参数     void
// 使用示例     ips200_show_rgb565_image_smooth2x(0, 0, scc8660_image[0], SCC8660_W, SCC8660_H);
// 备注信息     按 2 倍放大显示，并对相邻像素做通道级平均插值，画面比直接放大更平滑，
//              数据字节序与 ips200_show_rgb565_image(color_mode=1) 一致（凌瞳字节交换格式）。
//-------------------------------------------------------------------------------------------------------------------
void ips200_show_rgb565_image_smooth2x (uint16 x, uint16 y, const uint16 *image, uint16 width, uint16 height)
{
    zf_assert(x < ips200_x_max);
    zf_assert(y < ips200_y_max);
    zf_assert(NULL != image);

    uint16 dis_width  = (uint16)(width * 2);
    uint16 dis_height = (uint16)(height * 2);
    uint16 data_buffer[320];

    IPS200_CS(0);

    ips200_set_region(x, y, (uint16)(x + dis_width - 1u), (uint16)(y + dis_height - 1u));

    for(uint16 j = 0; j < dis_height; j ++)
    {
        uint16 sy = (uint16)(j >> 1);
        uint16 fy = (uint16)(j & 1);
        const uint16 *row0 = image + (uint32)sy * width;
        const uint16 *row1 = ((sy + 1u) < height) ? (row0 + width) : row0;

        for(uint16 i = 0; i < dis_width; i ++)
        {
            uint16 sx = (uint16)(i >> 1);
            uint16 fx = (uint16)(i & 1);

            /* 先把源像素从"高字节在前"的内存字节序换回标准 RGB565，再做通道平均 */
            uint32 p00 = ips200_rgb565_mem_to_std(row0[sx]);
            uint32 p10 = ((sx + 1u) < width) ? ips200_rgb565_mem_to_std(row0[sx + 1u]) : p00;
            uint32 p01 = ips200_rgb565_mem_to_std(row1[sx]);
            uint32 p11 = ((sx + 1u) < width) ? ips200_rgb565_mem_to_std(row1[sx + 1u]) : p01;

            if(fx && fy)
            {
                data_buffer[i] = ips200_rgb565_std_to_mem(ips200_rgb565_channel_avg4(p00, p10, p01, p11));
            }
            else if(fx)
            {
                data_buffer[i] = ips200_rgb565_std_to_mem(ips200_rgb565_channel_avg2(p00, p10));
            }
            else if(fy)
            {
                data_buffer[i] = ips200_rgb565_std_to_mem(ips200_rgb565_channel_avg2(p00, p01));
            }
            else
            {
                data_buffer[i] = ips200_rgb565_std_to_mem(p00);   /* 换两次等于原值，与 color_mode=1 直写一致 */
            }
        }
        ips200_write_8bit_data_spi_array((uint8 *)data_buffer, (uint32)dis_width * 2);
    }
...
void ips200_show_wave (uint16 x, uint16 y, const uint16 *wave, uint16 width, uint16 value_max, uint16 dis_width, uint16 dis_value_max)
{
    ...
    ips200_set_region(x, y, (uint16)(x + dis_width - 1u), (uint16)(y + dis_value_max - 1u));          // 设置显示区域
    ...
    {
        width_index = i * width / dis_width;
        value_max_index = (uint32)(*(wave + width_index)) * (uint32)(dis_value_max - 1u) / value_max;
        ips200_draw_point((uint16)(i + x), (uint16)((dis_value_max - 1u) - value_max_index + y), ips200_pencolor);
    }
}
...
void ips200_show_chinese (uint16 x, uint16 y, uint8 size, const uint8 *chinese_buffer, uint8 number, const uint16 color)
{
    ...
    ips200_set_region(x, y, (uint16)(number * size - 1u + x), (uint16)(y + size - 1u));
...
void sci5_b_spi_callback (spi_callback_args_t* p_args)
{
    switch(p_args->event)
    {
        case SPI_EVENT_TRANSFER_COMPLETE:
            ips200_spi_transfer_finish = true;
            break;
