...
/* 本文件中浮点与 0.0 的精确比较均为有意为之（浮点转字符串时判断是否为零），
 * 此处抑制 -Wfloat-equal 警告。 */
...
void func_int_to_str (char *str, int32 number)
{
    ...
    do
    {
        ...
        while(0 != number)                                                      // 循环直到数值归零
        {
            ...
            data_temp[bit ++] = (uint8)func_abs(number_temp);                          // 倒序将数值提取出来
            number /= 10;                                                       // 削减被提取的个位数
        }
        while(0 != bit)                                                         // 提取的数字个数递减处理
        {
            *str ++ = (char)(data_temp[bit - 1] + 0x30);                              // 将数字从倒序数组中倒序取出 变成正序放入字符串
            bit --;
        }
    }while(0);
}
...
void func_uint_to_str (char *str, uint32 number)
{
    ...
    do
    {
        ...
        while(0 != number)                                                      // 循环直到数值归零
        {
            data_temp[bit ++] = (int8)(number % 10u);                                  // 倒序将数值提取出来
            number /= 10;                                                       // 削减被提取的个位数
        }
        while(0 != bit)                                                         // 提取的数字个数递减处理
        {
...
void func_float_to_str (char *str, float number, uint8 point_bit)
{
    ...
    do
    {
        ...
        // 提取小数部分
        number = number - (float)data_int;                                      // 减去整数部分即可
        while(bit --)
        {
        ...
        while(0 != bit)
        {
            *str ++ = (char)(func_abs(data_temp[bit - 1]) + 0x30);                    // 再倒序将倒序的数值写入字符串 得到正序数值
            bit --;
        }
        ...
        if(0 != point_bit)
        {
            ...
            *str ++ = '.';
            ...
            else
            {
                ...
                while(0 != bit)
                {
                    *str ++ = (char)(func_abs(data_temp_point[bit - 1]) + 0x30);      // 再倒序将倒序的数值写入字符串 得到正序数值
                    bit --;
                }
            }
        }
        ...
        if ('%' == ret)
        {
            switch (*++ format)
            {
                ...
                case 'p':// 以16进制形式输出指针
                {
                    int8 vstr[33];
                    uint32 ival = (uint32)va_arg(arg, uint32);
                    (void)number_conversion_ascii(ival, vstr, 0, 16);
