...
static void debug_uart_callback (uart_callback_args_t * p_args)
{
    if(p_args->channel == 9)
    {
        switch(p_args->event)
...
__attribute__((weak)) int _read(int fd, char *pBuffer, int size)
{
    ...
    g_uart9.p_api->read(g_uart9.p_ctrl, (uint8_t *)pBuffer, 1U);
...
__attribute__((weak)) int _write (int fd, char * pBuffer, int size)
{
    ...
    g_uart9.p_api->write(g_uart9.p_ctrl, (uint8_t const *)pBuffer, (uint32_t)size);
