...
// Hardware Interface
#define soft_iic_gpio_high(pin)             g_ioport.p_api->pinWrite(&g_ioport_ctrl, pin, BSP_IO_LEVEL_HIGH)
#define soft_iic_gpio_low(pin)              g_ioport.p_api->pinWrite(&g_ioport_ctrl, pin, BSP_IO_LEVEL_LOW)
#define soft_iic_gpio_get_level(pin, dat)   g_ioport.p_api->pinRead(&g_ioport_ctrl, pin, dat)
...
void soft_iic_start (soft_iic_info_struct *soft_iic_obj)
{
    ...
    soft_iic_gpio_high(soft_iic_obj->scl_pin);                                           // SCL 高电平
    soft_iic_gpio_high(soft_iic_obj->sda_pin);                                           // SDA 高电平

    soft_iic_delay(soft_iic_obj->delay);
    soft_iic_gpio_low(soft_iic_obj->sda_pin);                                            // SDA 先拉低
    soft_iic_delay(soft_iic_obj->delay);
    soft_iic_gpio_low(soft_iic_obj->scl_pin);                                            // SCL 再拉低
    soft_iic_delay(soft_iic_obj->delay);
}
...
{
    zf_assert(NULL != soft_iic_obj);
    soft_iic_gpio_low(soft_iic_obj->sda_pin);                                            // SDA 低电平
    soft_iic_gpio_low(soft_iic_obj->scl_pin);                                            // SCL 低电平

    soft_iic_delay(soft_iic_obj->delay);
    soft_iic_gpio_high(soft_iic_obj->scl_pin);                                           // SCL 先拉高
    soft_iic_delay(soft_iic_obj->delay);
    soft_iic_gpio_high(soft_iic_obj->sda_pin);                                           // SDA 再拉高
    soft_iic_delay(soft_iic_obj->delay);
}
...
static void soft_iic_send_ack (soft_iic_info_struct *soft_iic_obj, uint8 ack)
{
    ...
    if(ack)
    {
        soft_iic_gpio_high(soft_iic_obj->sda_pin);                                       // SDA 拉高
    }
    else
    {
        soft_iic_gpio_low(soft_iic_obj->sda_pin);                                        // SDA 拉低
    }

    soft_iic_delay(soft_iic_obj->delay);
    soft_iic_gpio_high(soft_iic_obj->scl_pin);                                           // SCL 拉高
    soft_iic_delay(soft_iic_obj->delay);
    soft_iic_gpio_low(soft_iic_obj->scl_pin);                                            // SCL 拉低
    soft_iic_gpio_high(soft_iic_obj->sda_pin);                                           // SDA 拉高
}
...
static uint8 soft_iic_wait_ack (soft_iic_info_struct *soft_iic_obj)
{
    ...
    uint8 temp = 0;
    soft_iic_gpio_low(soft_iic_obj->scl_pin);                                            // SCL 低电平
    soft_iic_gpio_high(soft_iic_obj->sda_pin);                                           // SDA 高电平 释放 SDA
    ...
    uint8 data = 0;
    soft_iic_gpio_get_level(soft_iic_obj->sda_pin, &data);
    if(data)
    {
        temp = 1;
    }
...
//        gpio_set_level(soft_iic_obj->sda_pin, data & temp);
        ((data & temp) ? (soft_iic_gpio_high(soft_iic_obj->sda_pin)) : (soft_iic_gpio_low(soft_iic_obj->sda_pin)));
        temp >>= 1;
...
uint8 soft_iic_read_data (soft_iic_info_struct *soft_iic_obj, uint8 ack)
{
    ...
    while(temp --)
    {
        ...
        uint8 dat = 0;
        soft_iic_gpio_get_level(soft_iic_obj->sda_pin, &dat);
        data = ((data << 1) | dat);
    }
    soft_iic_gpio_low(soft_iic_obj->scl_pin);                                            // SCL 低电平
#if SOFT_IIC_SDA_IO_SWITCH
...
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
void soft_iic_init (soft_iic_info_struct *soft_iic_obj, uint8 addr, uint32 delay, uint32 scl_pin, uint32 sda_pin)
{
    zf_assert(NULL != soft_iic_obj);
